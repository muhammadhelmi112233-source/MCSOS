...
void pmm_zero_state(struct pmm_state *pmm) {
    if (pmm == (struct pmm_state *)0) {
        return;
    }
    pmm->bitmap         = (uint8_t *)0;
    pmm->bitmap_bytes   = 0;
    pmm->max_phys       = 0;
    pmm->frame_count    = 0;
    pmm->free_frames    = 0;
...
bool pmm_init_from_map(struct pmm_state *pmm,
    ...
    if (pmm == (struct pmm_state *)0 ||
        regions == (const struct boot_mem_region *)0 ||
        bitmap_storage == (uint8_t *)0 ||
        region_count == 0) {
        return false;
    }
    ...
    for (size_t i = 0; i < region_count; i++) {
        if (regions[i].type == (uint32_t)BOOT_MEM_USABLE) {
...
uint64_t pmm_alloc_frame(struct pmm_state *pmm) {
    if (pmm == (struct pmm_state *)0 || !pmm->initialized || pmm->free_frames == 0) {
...
bool pmm_free_frame(struct pmm_state *pmm, uint64_t phys_addr) {
    if (pmm == (struct pmm_state *)0 || !pmm->initialized) {
        return false;
    }
    if ((phys_addr & (PMM_PAGE_SIZE - 1ULL)) != 0 ||
        phys_addr == 0 ||
...
bool pmm_reserve_range(struct pmm_state *pmm, uint64_t base, uint64_t length) {
    if (pmm == (struct pmm_state *)0 || !pmm->initialized || length == 0) {
        return false;
    }
...
bool pmm_is_frame_free(const struct pmm_state *pmm, uint64_t phys_addr) {
    if (pmm == (const struct pmm_state *)0 || !pmm->initialized) {
        return false;
    }
...
uint64_t pmm_free_count(const struct pmm_state *pmm) {
    return (pmm != (const struct pmm_state *)0) ? pmm->free_frames : 0ULL;
}

uint64_t pmm_used_count(const struct pmm_state *pmm) {
    return (pmm != (const struct pmm_state *)0) ? pmm->used_frames : 0ULL;
}

uint64_t pmm_frame_count(const struct pmm_state *pmm) {
    return (pmm != (const struct pmm_state *)0) ? pmm->frame_count : 0ULL;
}
