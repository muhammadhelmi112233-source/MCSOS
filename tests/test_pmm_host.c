...
/* Gunakan pmm.h dari kernel/include dengan path eksplisit */
...
int main(void) {
    ...
    /* CP: double free ditolak */
    assert(!pmm_free_frame(&pmm, frame));

    /* CP: reserve range mengunci frame */
    ...
    /* CP: free frame 0 ditolak */
    assert(!pmm_free_frame(&pmm, 0));

    /* CP: free alamat non-aligned ditolak */
    assert(!pmm_free_frame(&pmm, 0x00100001ULL));

    /* CP: invariant free + used == frame_count */
    assert(pmm_free_count(&pmm) + pmm_used_count(&pmm) == pmm_frame_count(&pmm));
