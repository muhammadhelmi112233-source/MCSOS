...
int main(void) {
    ...
    /* CP: map satu halaman higher-half */
    ...
    /* CP: query mengembalikan paddr dan flags yang benar */
    ...
    /* CP: duplicate map ditolak */
    assert(vmm_map_page(&space, 0xFFFF800000200000ULL, 0x0000000000400000ULL, 0)
           == VMM_ERR_EXISTS);

    /* CP: paddr tidak aligned ditolak */
    assert(vmm_map_page(&space, 0xFFFF800000201000ULL, 0x0000000000400001ULL, 0)
           == VMM_ERR_INVAL);

    /* CP: noncanonical vaddr ditolak */
    assert(vmm_map_page(&space, 0x0000800000000000ULL, 0x0000000000400000ULL, 0)
           == VMM_ERR_INVAL);

    /* CP: unmap berhasil */
    assert(vmm_unmap_page(&space, 0xFFFF800000200000ULL) == VMM_MAP_OK);

    /* CP: query setelah unmap mengembalikan NOT_FOUND */
    assert(vmm_query_page(&space, 0xFFFF800000200000ULL, &m) == VMM_ERR_NOT_FOUND);

    /* CP: double unmap ditolak */
    assert(vmm_unmap_page(&space, 0xFFFF800000200000ULL) == VMM_ERR_NOT_FOUND);

    /* CP: map lower-half canonical */
