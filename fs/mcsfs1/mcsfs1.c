...
static int valid_name(const char *name, uint32_t *len_out) {
    ...
    if (n == 0u) return MCSFS1_ERR_INVAL;
    if (n > MCSFS1_MAX_NAME) return MCSFS1_ERR_NAMETOOLONG;
    for (uint32_t i = 0; i < n; i++) {
        if (name[i] == '/') return MCSFS1_ERR_INVAL;
    }
    *len_out = n;
...
static int dev_flush(struct mcsfs1_blkdev *dev) {
    if (dev == 0 || dev->flush == 0) return MCSFS1_ERR_INVAL;
...
static int read_inode(struct mcsfs1_blkdev *dev, uint32_t ino, struct mcsfs1_inode_disk *inode) {
    if (ino == 0u || ino > MCSFS1_MAX_INODES || inode == 0) return MCSFS1_ERR_INVAL;
    ...
    if (lba >= MCSFS1_DATA_START_LBA) return MCSFS1_ERR_CORRUPT;
...
static int find_dirent(struct mcsfs1_blkdev *dev, const char *name, uint32_t *slot_out, uint32_t *ino_out) {
    ...
    for (uint32_t i = 0; i < MCSFS1_DIRENT_COUNT; i++) {
        if (de[i].ino != 0u &&
            ...
            if (slot_out != 0) *slot_out = i;
            if (ino_out  != 0) *ino_out  = de[i].ino;
            return MCSFS1_ERR_OK;
        }
    }
...
static int alloc_inode_block(struct mcsfs1_blkdev *dev, uint32_t *ino_out, uint32_t *data_lba_out) {
    ...
    for (uint32_t i = 2u; i <= MCSFS1_MAX_INODES; i++) {
        if (!bit_test(ib, i)) { ino = i; break; }
    }
    if (ino == 0u) return MCSFS1_ERR_NOSPC;
    uint32_t lba = 0;
    for (uint32_t b = MCSFS1_DATA_START_LBA; b < dev->block_count; b++) {
        if (!bit_test(bb, b)) { lba = b; break; }
    }
    if (lba == 0u) return MCSFS1_ERR_NOSPC;
    bit_set(ib, ino);
    bit_set(bb, lba);
...
int mcsfs1_format(struct mcsfs1_blkdev *dev) {
    ...
    for (uint32_t b = 0; b < MCSFS1_DATA_START_LBA; b++) bit_set(bb, b);
...
int mcsfs1_mount(struct mcsfs1_mount *mnt, struct mcsfs1_blkdev *dev) {
    if (mnt == 0 || dev == 0) return MCSFS1_ERR_INVAL;
    struct mcsfs1_super_disk sb;
...
int mcsfs1_create(struct mcsfs1_mount *mnt, const char *name) {
    if (mnt == 0 || mnt->dev == 0) return MCSFS1_ERR_INVAL;
    uint32_t name_len = 0;
    int rc = valid_name(name, &name_len);
    if (rc != 0) return rc;
    if (find_dirent(mnt->dev, name, 0, 0) == 0) return MCSFS1_ERR_EXIST;
    ...
    for (uint32_t i = 0; i < MCSFS1_DIRENT_COUNT; i++) {
        if (de[i].ino == 0u) { free_slot = i; break; }
    }
    if (free_slot == MCSFS1_DIRENT_COUNT) return MCSFS1_ERR_NOSPC;
    uint32_t ino = 0;
...
int mcsfs1_write(struct mcsfs1_mount *mnt, const char *name, const uint8_t *buf, uint32_t len) {
    if (mnt == 0 || mnt->dev == 0 || (buf == 0 && len != 0u)) return MCSFS1_ERR_INVAL;
    if (len > MCSFS1_DIRECT_BLOCKS * MCSFS1_BLOCK_SIZE) return MCSFS1_ERR_RANGE;
    uint32_t ino = 0;
    ...
    if (rc != 0) return rc;
    if (inode.mode != MCSFS1_MODE_FILE) return MCSFS1_ERR_ISDIR;
    ...
    for (uint32_t i = 0; i < blocks_needed; i++) {
        ...
        if (chunk != 0u) mcsfs_memcpy(block, buf + written, chunk);
...
int mcsfs1_read(struct mcsfs1_mount *mnt, const char *name, uint8_t *buf, uint32_t cap, uint32_t *out_len) {
    if (mnt == 0 || mnt->dev == 0 || buf == 0 || out_len == 0) return MCSFS1_ERR_INVAL;
    uint32_t ino = 0;
    int rc = find_dirent(mnt->dev, name, 0, &ino);
    if (rc != 0) return rc;
    struct mcsfs1_inode_disk inode;
    ...
    if (rc != 0) return rc;
    if (inode.mode != MCSFS1_MODE_FILE) return MCSFS1_ERR_ISDIR;
    if (cap < inode.size) return MCSFS1_ERR_RANGE;
...
int mcsfs1_fsck(struct mcsfs1_blkdev *dev) {
    if (dev == 0) return MCSFS1_ERR_INVAL;
    struct mcsfs1_super_disk sb;
    int rc = load_super(dev, &sb);
    if (rc != 0) return rc;
    uint8_t ib[MCSFS1_BLOCK_SIZE];
    ...
    for (uint32_t b = 0; b < MCSFS1_DATA_START_LBA; b++) {
        if (!bit_test(bb, b)) return MCSFS1_ERR_CORRUPT;
    }
    struct mcsfs1_inode_disk root;
    rc = read_inode(dev, MCSFS1_ROOT_INO, &root);
    if (rc != 0) return rc;
